...
// _shared/auth-retry.ts
var AUTH_RETRY_DELAYS_MS = [250, 750];
var wait = /* @__PURE__ */ __name((ms) => new Promise((resolve) => setTimeout(resolve, ms)), "wait");
var getSupabaseUserWithRetry = /* @__PURE__ */ __name(async (admin, token) => {
...
var parseJsonSafely = /* @__PURE__ */ __name((text) => {
  if (!text) return null;
  try {
...
var resolveStripeSecretKey = /* @__PURE__ */ __name((env) => {
...
var requireGoogleUser = /* @__PURE__ */ __name(async (request, env, corsHeaders2) => {
  ...
  if (!isGoogleUser(data.user)) {
    return { response: jsonResponse({ error: "Google\u30ED\u30B0\u30A4\u30F3\u306E\u307F\u5229\u7528\u3067\u304D\u307E\u3059\u3002" }, 403, corsHeaders2) };
  }
...
var onRequestPost = /* @__PURE__ */ __name(async ({ request, env }) => {
  ...
  params.set("metadata[app]", "orcaai");
  params.set("payment_intent_data[statement_descriptor]", "ORCAAI");
  let stripeRes;
  try {
    ...
    return jsonResponse({ error: "Stripe API\u3078\u306E\u63A5\u7D9A\u306B\u5931\u6557\u3057\u307E\u3057\u305F\u3002" }, 502, corsHeaders2);
  }
  const stripeText = await stripeRes.text();
  const stripeData = parseJsonSafely(stripeText);
  if (!stripeRes.ok) {
    const stripeMessage = stripeData?.error?.message || (typeof stripeText === "string" && stripeText.trim() ? stripeText.trim().slice(0, 300) : "");
    return jsonResponse({ error: stripeMessage || "Stripe\u306E\u30BB\u30C3\u30B7\u30E7\u30F3\u4F5C\u6210\u306B\u5931\u6557\u3057\u307E\u3057\u305F\u3002" }, 500, corsHeaders2);
  }
  const checkoutUrl = typeof stripeData?.url === "string" ? stripeData.url : "";
  if (!checkoutUrl) {
    return jsonResponse({ error: "Stripe\u30BB\u30C3\u30B7\u30E7\u30F3URL\u306E\u53D6\u5F97\u306B\u5931\u6557\u3057\u307E\u3057\u305F\u3002" }, 500, corsHeaders2);
  }
  return jsonResponse({ url: checkoutUrl }, 200, corsHeaders2);
}, "onRequestPost");

// api/stripe/webhook.ts
var PRICE_MAP2 = /* @__PURE__ */ new Map([
  ["price_1TIA1SAHjIANZ9z3a3U015UN", { label: "\u304A\u8A66\u3057\u30D1\u30C3\u30AF", tickets: 25 }],
  ["price_1TIA1jAHjIANZ9z3ReE5aAsV", { label: "\u304A\u5F97\u30D1\u30C3\u30AF", tickets: 115 }],
  ["price_1TIA2LAHjIANZ9z3uNOI1ZQr", { label: "\u5927\u5BB9\u91CF\u30D1\u30C3\u30AF", tickets: 600 }]
]);
...
var resolveStripeWebhookSecret = /* @__PURE__ */ __name((env) => {
...
var onRequestPost2 = /* @__PURE__ */ __name(async ({ request, env }) => {
  ...
  if (event.type !== "checkout.session.completed") {
    return jsonResponse2({ received: true });
  }
  ...
  const plan = PRICE_MAP2.get(priceId);
  if (!priceId || !plan) {
    return jsonResponse2({ received: true });
  }
  ...
  if (result?.already_processed) {
    return jsonResponse2({ received: true, duplicate: true });
  }
  return jsonResponse2({ received: true });
}, "onRequestPost");
...
var wan_workflow_i2v_default = {
  ...
  "99": {
    ...
    inputs: {
      ...
      frame_rate: 10,
      loop_count: 0,
      filename_prefix: "video/orcaai_i2v",
  ...
  "77": {
    ...
    inputs: {
      unet_name: "wan22EnhancedNSFWSVICamera_nsfwFASTMOVEV2FP8H.safetensors",
  ...
  "103": {
    class_type: "UNETLoader",
    inputs: {
      unet_name: "wan22EnhancedNSFWSVICamera_nsfwFASTMOVEV2FP8L.safetensors",
      weight_dtype: "default"
    }
  },
  "111": {
...
// api/wan-dasiwa-workflow-i2v.json
var wan_dasiwa_workflow_i2v_default = {
  "6": {
    class_type: "CLIPTextEncode",
    inputs: {
      clip: [
...
// api/wan-workflow-animate.json
var wan_workflow_animate_default = {
  "1": {
    class_type: "CLIPTextEncode",
    inputs: {
  ...
  "432": {
    ...
    inputs: {
      ...
      loop_count: 0,
      filename_prefix: "wan2/anime-seg",
      format: "video/nvenc_h264-mp4",
      pix_fmt: "yuv420p",
      bitrate: 10,
      megabit: true,
  ...
  "487": {
    ...
    _meta: {
      title: "easy forLoopStart"
    }
  },
  "507": {
    class_type: "VHS_LoadVideo",
    inputs: {
      video: "LTX2_00008.mp4",
      force_rate: 0,
      custom_width: 0,
      custom_height: 0,
      frame_load_cap: 0,
  ...
  "556": {
    ...
    inputs: {
      ...
      loop_count: 0,
      filename_prefix: "wan2/anime-mask",
      format: "video/nvenc_h264-mp4",
      pix_fmt: "yuv420p",
      bitrate: 10,
      megabit: true,
  ...
  "242:552": {
    class_type: "ImpactConditionalBranchSelMode",
    inputs: {
      cond: [
        "242:551",
        1
    ...
    _meta: {
      title: "ImpactConditionalBranchSelMode"
    }
  },
  "242:553": {
    class_type: "ImpactConditionalBranchSelMode",
  ...
  "490:486": {
    ...
    inputs: {
      model_name: "Wan2.2-Animate-14B-Q4_K_M.gguf",
  ...
  "490:18": {
    class_type: "LoraLoaderModelOnly",
    inputs: {
      lora_name: "wan21-lightx2v-i2v-14b-480p-cfg-step-distill-rank64-bf16.safetensors",
      strength_model: 1,
      model: [
        "490:486",
        0
...
// api/wan-node-map-animate.json
var wan_node_map_animate_default = {
  image: { id: "10", input: "image" },
  video: { id: "507", input: "video" },
  prompt: { id: "21", input: "text" },
  negative_prompt: { id: "1", input: "text" },
  seed: { id: "522", input: "seed" },
  steps: [
    { id: "232:63", input: "steps" },
    { id: "242:91", input: "steps" }
  ],
  cfg: [
    { id: "232:63", input: "cfg" },
    { id: "242:91", input: "cfg" }
  ],
  width: { id: "516:515", input: "width" },
  height: { id: "516:515", input: "height" },
  num_frames: { id: "516:515", input: "length" },
  fps: { id: "507", input: "force_rate" }
};

// _shared/rekognition.ts
var isUnderageImage = /* @__PURE__ */ __name(async (_base64, _env) => {
  return false;
}, "isUnderageImage");

// _shared/wan-lora-pack-handler.ts
var corsMethods2 = "POST, GET, OPTIONS";
var jsonResponse3 = /* @__PURE__ */ __name((body, status = 200, headers = {}) => new Response(JSON.stringify(body), {
  status,
  headers: { ...headers, "Content-Type": "application/json" }
}), "jsonResponse");
var DEFAULT_LORA_PACK_ENDPOINT = "https://api.runpod.ai/v2/rywgsws0odibjj";
var DEFAULT_LORA_PACK_STRENGTH = 0;
var MIN_LORA_PACK_STRENGTH = 0;
var ACTIVE_LORA_PACK_STRENGTH = 0.1;
var MAX_LORA_PACK_STRENGTH = 1.5;
var MAX_LORA_PACK_SELECTIONS = 40;
var LORA_PACK_OPTIONS = {
...
var SCREEN_FLOOD_LORA_ID = "e6b7b9e6b2a1";
var SCREEN_FLOOD_TRIGGER = "yanmo567";
var SCREEN_FLOOD_PROMPT = "a huge splash of water erupts from the bottom and submerges the entire screen.";
var LIFT_ONE_LEG_LORA_ID = "low_e9ab98e68aace885";
var LIFT_ONE_LEG_PROMPT = "They lifted one of their legs high up.";
var FACE_DOWN_ASS_UP_LORA_ID = "facedownassup";
var FACE_DOWN_ASS_UP_PROMPT = "A naked woman is having sex with a man in the face-down ass-up position. She is having sex with a man in the top-down bottom-up position";
var SUSPENDED_CONGRESS_LORA_ID = "reverse_suspended_congress";
var SUSPENDED_CONGRESS_PROMPT = "A woman is having sex in the reverse_suspended_congress position She spreads her legs and her body moves up and down, while the man thrusts his penis in and out of her vaginal.";
var HIT_THE_FACE_LORA_ID = "e68993e884b890e9ab98";
var HIT_THE_FACE_PROMPT = "dalian666,Suddenly, a baseball bat hits her face from the right side. Her head jerks sharply to the left due to the impact, and her facial expression is shocked and distorted from the external force, but there is no blood or gore. This moment is captured at the instant of impact, with motion blur on the hair and slight facial deformation, creating a dynamic, cinematic freeze-frame with a shallow depth of field and dramatic lighting effects.";
var FRENCH_KISS_LORA_ID = "frenchkiss";
var FRENCH_KISS_PROMPT = "Two people kiss.";
var GLASS_KISS_LORA_ID = "e78ebbe79283";
var GLASS_KISS_PROMPT = "boli567,a woman is holding a transparent piece of glass, kissing it so affectionately that saliva drips down the surface.";
var CATCH_POSE_LORA_ID = "e68a93e4bd8fe8b7aa";
var CATCH_POSE_PROMPT = "First-person perspective, a hand from outside reaches out to grab their neck, forcing them to kneel.";
var POV_MISSIONARY_LORA_ID = "pov_missionary";
var POV_MISSIONARY_PROMPT = "with her legs spread having sex with a man, A man is thrusting his penis back and forth inside her vagina at the bottom of the screen";
var CUMSHOT_1_LORA_ID = "cumshot_aesthetics_1";
var CUMSHOT_1_PROMPT = "An adult woman is kneeling. A man enters the frame from the bottom left corner. He ejaculates on her face and into her mouth, with a small amount landing on her nose. The fluid is thick and sticky, clinging like paste before sliding off.";
var CUMSHOT_2_LORA_ID = "cumshot_aesthetics_2";
var CUMSHOT_2_PROMPT = "An adult woman is sitting. A naked man enters from the right side. He ejaculates on her chest. Thick and heavy, the fluid holds to her body before dripping away.";
var CUMSHOT_3_LORA_ID = "cumshot_aesthetics_3";
var CUMSHOT_3_PROMPT = "An adult woman is standing. A man enters from below. He ejaculates on her mouth. The sticky fluid briefly clings before slowly sliding off her skin.";
var CUMSHOT_4_LORA_ID = "cumshot_aesthetics_4";
var CUMSHOT_4_PROMPT = "An adult woman is lying down. A man enters from below. He ejaculates on her stomach. It is thick and sticky, clinging like paste before sliding off slowly.";
var BLINK_TITJOB_1_LORA_ID = "blink_titjob_1";
var BLINK_TITJOB_1_PROMPT = "The video begins with a close up of an adult woman. The video then jumpcuts to the same adult woman now lying down on a tiled floor of the same location with her breasts positioned around the man's erect penis as he thrusts his penis up and down in a titjob motion sliding it between her breasts. She makes various facial expressions during the video, she looks like she is talking and has her eyes wide open with a crazy expression.";
var BLINK_TITJOB_2_LORA_ID = "blink_titjob_2";
var BLINK_TITJOB_2_PROMPT = "The video begins with a close up of an adult woman. The video then jumpcuts to the same adult woman kneeling in the same location with her breasts positioned around the man's erect penis as she moves them up and down in a sliding motion. She makes various facial expressions, she looks like she is talking and has her eyes wide open with a crazy expression.";
var BLINK_BACK_DOGGYSTYLE_1_LORA_ID = "blink_back_doggystyle_1";
var BLINK_BACK_DOGGYSTYLE_1_PROMPT = "The video begins with a shot of an adult woman. The video then jumpcuts to the same adult woman now having sex in doggystyle position. She is positioned kneeling in the same location. The video is shot from behind as she looks back at the camera with an open mouth expression. He penetrates her vagina from behind. Her legs are close together with the man kneeling behind her over her legs. The man has a wide stance. She is looking directly at the camera fully facing it. She looks back at the camera. She looks at the camera throughout the video.";
var BLINK_BACK_DOGGYSTYLE_2_LORA_ID = "blink_back_doggystyle_2";
var BLINK_BACK_DOGGYSTYLE_2_PROMPT = "The video begins with shot of an adult woman. The video then jumpcuts to the same adult woman now having sex in doggystyle position in the same location. From an overhead perspective, she is on all fours with her back facing the camera. A man is positioned behind her, his hands gripping her hips as he penetrates her from behind. The adult woman's expression changes throughout the scene, showing moments of pleasure and engagement with her partner. Her legs are spread apart with the man in-between her legs. She is looking directly at the camera fully facing it. She looks back at the camera. She looks at the camera throughout the video.";
var BLINK_FACIAL_1_LORA_ID = "blink_facial_1";
var BLINK_FACIAL_1_PROMPT = "The video begins with a close-up of an adult woman. The video then jumpcuts to the same adult woman now receiving a facial from a man's penis. She is kneeling on the floor looking up with an open mouth. The cum shoots all over her face. The man's hand holds his erect penis, masturbating his penis and shooting the thick white cum directly onto her face, forehead, eyes, cheek and mouth. The thick white cum slowly drips down her face onto her body. An explosion of thick white cum blasts her face. She looks directly at the camera throughout the video.";
var BLINK_FACIAL_2_LORA_ID = "blink_facial_2";
var BLINK_FACIAL_2_PROMPT = "The video begins with a close-up of an adult woman. The video then jumpcuts to the same adult woman now receiving a facial from a man's penis. She is lying on her back. The cum shoots all over her face. The man's hand holds his erect penis, masturbating his penis and shooting the thick white cum directly onto her face, forehead, eyes, cheek and mouth. The thick white cum slowly drips down her face onto her body. An explosion of thick white cum blasts her face. She looks directly at the camera throughout the video.";
var BLINK_HANDJOB_1_LORA_ID = "blink_handjob_1";
var BLINK_HANDJOB_1_PROMPT = "The video begins with a close-up of an adult woman. The video then jumpcuts to the same adult woman on her stomach with her head resting low in the frame on a man's thigh next to his penis on the left of the frame. With her right hand she grasps the man's erect penis and moves it up and down the shaft in a steady rhythm, performing the handjob. She goes through various facial expressions throughout the video from happy to gasping, she looks like she is talking. She looks at the camera throughout the video.";
var BLINK_HANDJOB_2_LORA_ID = "blink_handjob_2";
var BLINK_HANDJOB_2_PROMPT = "The video begins with a close-up of an adult woman. The video then jumpcuts to the same adult woman now kneeling between a man's legs with her upper body bent forward over him, and her face close to his lap. With one hand, she grasps the man's erect penis and moves it up and down its shaft in a steady rhythm, performing the handjob. She goes through various facial expressions throughout the video from happy to gasping, she looks like she is talking. She looks at the camera throughout the video. The man's feet are seen in the background.";
var BLINK_HANDJOB_3_LORA_ID = "blink_handjob_3";
var BLINK_HANDJOB_3_PROMPT = "The video begins with close-up of an adult woman. The video then jumpcuts to the same adult woman now kneeling on the floor in front of the man who is sitting high above her. She is giving the man's penis a handjob with both hands moving them up and down along the shaft. She goes through various facial expressions throughout the video from happy to gasping, she looks like she is talking. The adult woman is looking up at the man. She looks at the camera throughout the video.";
var PAIZURI_LORA_ID = "pov_titfuck_paizuri";
var PAIZURI_PROMPT = "titJob, paizuri, nakedman and adult woman, gather, fingersTogether. A man is thrusting his penis between her breasts. She rubs her breasts up and down his penis. She gathers her breasts together around the penis, with her fingertips touching together.";
var I2PEE_LORA_ID = "i2pee";
var I2PEE_PROMPT = "piss";
var FRONT_DOGGY_1_LORA_ID = "blink_front_doggystyle_1";
var FRONT_DOGGY_1_PROMPT = "The video begins with a close-up of an adult woman. The video then jumpcuts to the same adult woman now having sex in doggystyle position. She is lying on her stomach, facing forward, with her head turned slightly to the side as she reacts to the sensations of intercourse. Her facial expressions change throughout the sequence, showing moments of pleasure and exertion, including wide eyes, an open mouth, and clenched fists.";
var FRONT_DOGGY_2_LORA_ID = "blink_front_doggystyle_2";
var FRONT_DOGGY_2_PROMPT = "The video begins with a close-up of an adult woman. The video then jumpcuts to the same adult woman now having sex with a man in doggystyle position in the same location. She is positioned standing, while the man stands behind her. The man is muscular, his hands are wrapped around the woman's stomach holding her upright while embracing her from behind, he holds her close as he thrusts into her. As the scene progresses, she moves rhythmically with him. She is fully nude. The man aggressively rams his hips into her.";
var FRONT_DOGGY_3_LORA_ID = "blink_front_doggystyle_3";
var FRONT_DOGGY_3_PROMPT = "The video begins with a close-up of an adult woman. The video then jumpcuts to the same adult woman now having sex with a man in doggystyle position in the same location. She is bent over with her back arched and her head tilted down as he stands behind her. He is muscular. The adult woman's expression changes throughout the sequence as she reacts to their movements, sometimes looking down, other times up at the camera with an open mouth. The scene is recorded from below looking up. She looks at the camera the entire time. The man aggressively has sex with her. She is fully nude.";
var BLINK_BLOWJOB_LORA_ID = "blink_blowjob";
var BLINK_BLOWJOB_PROMPT = "An adult woman looking at the camera. The video then jumpcuts to the same adult woman giving a blowjob to a man standing in the same location, looking up as she performs the blowjob on the man. She is kneeling in front of him, she is holding his penis with both hands. She looks at the camera the entire time. She shoves the penis deep in her mouth.";
var HANDJOB_BLOWJOB_COMBO_LORA_ID = "handjob_blowjob_combo";
var HANDJOB_BLOWJOB_COMBO_PROMPT = BLINK_BLOWJOB_PROMPT;
var BLINK_MISSIONARY_1_LORA_ID = "blink_missionary_1";
var BLINK_MISSIONARY_1_PROMPT = "The video begins with a close-up of an adult woman. The video then jumpcuts to the same adult woman now having sex in missionary position. She is lying on her back on a bed with a patterned bed spread and pillow with her legs spread wide. A man's large penis is visible entering her vagina from below. The man is positioned kneeling between her legs in front of her thrusting his penis into her vagina. Throughout the scene, she appears to be experiencing pleasure, often with her mouth open or eyes closed as she lies back. Her hands hold onto her thighs spreading her legs.";
var BLINK_MISSIONARY_2_LORA_ID = "blink_missionary_2";
var BLINK_MISSIONARY_2_PROMPT = "The video begins with a close-up of an adult woman. The video then jumpcuts to the same adult woman now having sex in missionary position. She is lying on her back on a bed with a patterned bed spread and pillow with her legs spread with her knees to her chest. A man's large penis is visible entering her vagina from below. The man is positioned kneeling between her legs in front of her thrusting his penis into her vagina. Throughout the scene, she appears to be experiencing pleasure, often with her mouth open or eyes closed as she lies back. Her hands hold onto her thighs spreading her legs.";
var BLINK_MISSIONARY_3_LORA_ID = "blink_missionary_3";
var BLINK_MISSIONARY_3_PROMPT = "The video begins with a close-up of an adult woman. The video then jumpcuts to the same adult woman having sex in missionary position on the edge of a bed with a man's erect penis in her vagina. She is shown experiencing various stages of sexual pleasure, with her facial expressions changing from contentment to intense enjoyment as the act continues. She looks at the camera throughout the video.";
var SQUATTING_COWGIRL_1_LORA_ID = "blink_squatting_cowgirl_1";
var SQUATTING_COWGIRL_1_PROMPT = "The video begins with a close-up of an adult woman. The video then jumpcuts to the same adult woman now having sex with a man in squatting cowgirl position. Her face fills the screen, she is leaning over forwards as she bounces up and down aggressively. His erect penis is in her vagina. She looks at the camera throughout the video. The video is shot from above looking down on the scene.";
var SQUATTING_COWGIRL_2_LORA_ID = "blink_squatting_cowgirl_2";
var SQUATTING_COWGIRL_2_PROMPT = "The video begins with a close-up of an adult woman. The video then jumpcuts to the same adult woman now having sex with a man in squatting cowgirl position. She is leaning backwards as she bounces up and down aggressively. His erect penis is in her vagina. She looks at the camera throughout the video.";
var SIDELEG_TRANSITION_LORA_ID = "sideleg_transition";
var SIDELEG_TRANSITION_PROMPT = "They are having side sex";
var isLoraPackRoute = /* @__PURE__ */ __name((request) => new URL(request.url).pathname.toLowerCase().includes("/api/wan-lora-pack"), "isLoraPackRoute");
var resolveEndpoint = /* @__PURE__ */ __name((env, request, mode = "i2v", i2vVariant = "default") => {
  if (mode === "i2v") {
    if (isLoraPackRoute(request)) {
      return (env.RUNPOD_WAN_LORA_PACK_ENDPOINT_URL ?? DEFAULT_LORA_PACK_ENDPOINT).replace(/\/$/, "");
    }
    const i2vEndpoint = i2vVariant === "dasiwa" ? env.RUNPOD_WAN_DASIWA_ENDPOINT_URL ?? env.RUNPOD_WAN_RAPID_FASTMOVE_ENDPOINT_URL ?? env.RUNPOD_WAN_ENDPOINT_URL ?? env.RUNPOD_ENDPOINT_URL : env.RUNPOD_WAN_RAPID_FASTMOVE_ENDPOINT_URL ?? env.RUNPOD_WAN_ENDPOINT_URL ?? env.RUNPOD_ENDPOINT_URL;
    return i2vEndpoint?.replace(/\/$/, "");
  }
  return (env.RUNPOD_WAN_ENDPOINT_URL ?? env.RUNPOD_ENDPOINT_URL)?.replace(/\/$/, "");
}, "resolveEndpoint");
var resolveI2VVariant = /* @__PURE__ */ __name((request) => {
  const path = new URL(request.url).pathname.toLowerCase();
  return path.includes("/api/wan-dasiwa") ? "dasiwa" : "default";
}, "resolveI2VVariant");
var SIGNUP_TICKET_GRANT = 3;
var DEFAULT_VIDEO_TICKET_COST = 1;
var MAX_IMAGE_BYTES = 10 * 1024 * 1024;
var MAX_VIDEO_BYTES = 150 * 1024 * 1024;
var MAX_PROMPT_LENGTH = 5e3;
var MAX_NEGATIVE_PROMPT_LENGTH = 500;
var FIXED_STEPS = 4;
var FIXED_STEPS_ANIMATE = 6;
var LORA_PACK_STEPS = 4;
var LORA_PACK_SPLIT_STEP = 1;
var LORA_PACK_SHIFT = 2;
var MIN_DIMENSION = 256;
var MAX_DIMENSION = 3e3;
var MIN_CFG = 0;
var MAX_CFG = 10;
var FIXED_FPS_DEFAULT = 10;
var FIXED_FPS_DASIWA = 16;
var LORA_PACK_FPS = 10;
var VIDEO_DURATION_OPTIONS_DEFAULT = [
  { seconds: 5, frames: 53, ticketCost: 1 },
  { seconds: 7, frames: 73, ticketCost: 3 },
  { seconds: 10, frames: 101, ticketCost: 6 }
];
var VIDEO_DURATION_OPTIONS_DASIWA = [
  { seconds: 5, frames: 81, ticketCost: 1 },
  { seconds: 7, frames: 113, ticketCost: 3 },
  { seconds: 9, frames: 145, ticketCost: 5 }
];
var VIDEO_DURATION_OPTIONS_LORA_PACK = [
  { seconds: 5, frames: 53, ticketCost: 1 },
  { seconds: 7, frames: 73, ticketCost: 2 },
  { seconds: 10, frames: 101, ticketCost: 3 }
];
var FIXED_ANIMATE_FRAMES = 77;
var INTERNAL_SERVER_ERROR_MESSAGE = "\u30B5\u30FC\u30D0\u30FC\u5185\u90E8\u30A8\u30E9\u30FC\u304C\u767A\u751F\u3057\u307E\u3057\u305F\u3002\u6642\u9593\u3092\u304A\u3044\u3066\u518D\u5EA6\u304A\u8A66\u3057\u304F\u3060\u3055\u3044\u3002";
var ERROR_LOGIN_REQUIRED = "\u30ED\u30B0\u30A4\u30F3\u304C\u5FC5\u8981\u3067\u3059\u3002";
var ERROR_AUTH_FAILED = "\u8A8D\u8A3C\u306B\u5931\u6557\u3057\u307E\u3057\u305F\u3002";
var ERROR_GOOGLE_ONLY = "Google\u30ED\u30B0\u30A4\u30F3\u306E\u307F\u5BFE\u5FDC\u3057\u3066\u3044\u307E\u3059\u3002";
var ERROR_SUPABASE_NOT_SET = "SUPABASE_URL \u307E\u305F\u306F SUPABASE_SERVICE_ROLE_KEY \u304C\u8A2D\u5B9A\u3055\u308C\u3066\u3044\u307E\u305B\u3093\u3002";
var ERROR_ID_REQUIRED = "id\u304C\u5FC5\u8981\u3067\u3059\u3002";
var ERROR_JOB_NOT_FOUND = "Job not found.";
var ERROR_I2V_IMAGE_REQUIRED = "i2v\u306B\u306F\u753B\u50CF\u304C\u5FC5\u8981\u3067\u3059\u3002";
var ERROR_ANIMATE_VIDEO_REQUIRED = "animate\u306B\u306F\u53C2\u7167\u52D5\u753B\u304C\u5FC5\u8981\u3067\u3059\u3002";
var ERROR_IMAGE_READ_FAILED = "\u753B\u50CF\u306E\u8AAD\u307F\u53D6\u308A\u306B\u5931\u6557\u3057\u307E\u3057\u305F\u3002\u753B\u50CF\u3092\u78BA\u8A8D\u3057\u3066\u518D\u5EA6\u304A\u8A66\u3057\u304F\u3060\u3055\u3044\u3002";
var ERROR_VIDEO_READ_FAILED = "\u52D5\u753B\u306E\u8AAD\u307F\u53D6\u308A\u306B\u5931\u6557\u3057\u307E\u3057\u305F\u3002\u52D5\u753B\u3092\u78BA\u8A8D\u3057\u3066\u518D\u5EA6\u304A\u8A66\u3057\u304F\u3060\u3055\u3044\u3002";
var UNDERAGE_BLOCK_MESSAGE = "\u3053\u306E\u753B\u50CF\u306B\u306F\u66B4\u529B\u7684\u306A\u8868\u73FE\u3001\u4F4E\u5E74\u9F62\u3001\u307E\u305F\u306F\u898F\u7D04\u9055\u53CD\u306E\u53EF\u80FD\u6027\u304C\u3042\u308A\u307E\u3059\u3002\u5225\u306E\u753B\u50CF\u3067\u304A\u8A66\u3057\u304F\u3060\u3055\u3044\u3002";
var HIDDEN_MODEL_NAME_PATTERN = /Irodori-TTS-500M-v2-VoiceDesign|Irodori-TTS-500M-v2|Irodori-TTS|Irodori|VoiceDesign|MMAudio|Wav2Lip|WAV2LIP/gi;
var HIDDEN_MODEL_FILE_PATTERN = /[A-Za-z0-9_./%\-]+?\.(safetensors|gguf|onnx|pt|pth|ckpt|bin)/gi;
var GENERIC_MODEL_LABEL = "model";
var getVideoDurationOptions = /* @__PURE__ */ __name((i2vVariant, isLoraPack = false) => {
...
var getDefaultVideoDurationOption = /* @__PURE__ */ __name((i2vVariant, isLoraPack = false) => getVideoDurationOptions(i2vVariant, isLoraPack)[0], "getDefaultVideoDurationOption");
var resolveVideoDurationOption = /* @__PURE__ */ __name((seconds, i2vVariant, isLoraPack = false) => getVideoDurationOptions(i2vVariant, isLoraPack).find((option) => option.seconds === seconds) ?? null, "resolveVideoDurationOption");
var resolveVideoDurationFromInput = /* @__PURE__ */ __name((secondsRaw, i2vVariant, isLoraPack = false) => {
  if (secondsRaw === void 0 || secondsRaw === null || secondsRaw === "") {
    return { option: getDefaultVideoDurationOption(i2vVariant, isLoraPack), error: null };
  }
  const parsedSeconds = Math.floor(Number(secondsRaw));
  if (!Number.isFinite(parsedSeconds)) {
    return { option: null, error: "seconds must be a number." };
  }
...
var parseTicketMetadata = /* @__PURE__ */ __name((value) => {
  if (!value || typeof value !== "object" || Array.isArray(value)) return null;
  return value;
}, "parseTicketMetadata");
var normalizeEmail = /* @__PURE__ */ __name((value) => (value ?? "").trim().toLowerCase(), "normalizeEmail");
var isUsageOwnedByUser = /* @__PURE__ */ __name((event, user) => {
  if (event.user_id && event.user_id === user.id) return true;
  const userEmail = normalizeEmail(user.email ?? "");
  return Boolean(userEmail && normalizeEmail(event.email) === userEmail);
}, "isUsageOwnedByUser");
var fetchUsageEvent = /* @__PURE__ */ __name(async (admin, usageId) => {
  const { data, error } = await admin.from("ticket_events").select("usage_id, user_id, email, delta, metadata").eq("usage_id", usageId).maybeSingle();
  if (error || !data) {
...
var requireOwnedUsageChargeEvent = /* @__PURE__ */ __name(async (admin, user, usageId, corsHeaders2) => {
  const { event, error } = await fetchUsageEvent(admin, usageId);
  if (error) {
    return { response: jsonResponse3({ error: INTERNAL_SERVER_ERROR_MESSAGE }, 500, corsHeaders2) };
  }
  if (!event || !isUsageOwnedByUser(event, user) || Number(event.delta) >= 0) {
    return { response: jsonResponse3({ error: ERROR_JOB_NOT_FOUND }, 404, corsHeaders2) };
  }
  return { event };
}, "requireOwnedUsageChargeEvent");
var getWorkflowTemplate = /* @__PURE__ */ __name(async (mode, i2vVariant = "default") => mode === "animate" ? wan_workflow_animate_default : i2vVariant === "dasiwa" ? wan_dasiwa_workflow_i2v_default : wan_workflow_i2v_default, "getWorkflowTemplate");
var getNodeMap = /* @__PURE__ */ __name(async (mode) => mode === "animate" ? wan_node_map_animate_default : wan_node_map_i2v_default, "getNodeMap");
...
var isGoogleUser2 = /* @__PURE__ */ __name((user) => {
  if (user.app_metadata?.provider === "google") return true;
  if (Array.isArray(user.identities)) {
    return user.identities.some((identity) => identity.provider === "google");
  }
...
var requireGoogleUser2 = /* @__PURE__ */ __name(async (request, env, corsHeaders2) => {
  const token = extractBearerToken2(request);
  if (!token) {
    return { response: jsonResponse3({ error: ERROR_LOGIN_REQUIRED }, 401, corsHeaders2) };
  }
  const admin = getSupabaseAdmin3(env);
  if (!admin) {
    return { response: jsonResponse3({ error: ERROR_SUPABASE_NOT_SET }, 500, corsHeaders2) };
  }
  const { data, error } = await getSupabaseUserWithRetry(admin, token);
  if (error || !data?.user) {
    return { response: jsonResponse3({ error: ERROR_AUTH_FAILED }, 401, corsHeaders2) };
  }
  if (!isGoogleUser2(data.user)) {
    return { response: jsonResponse3({ error: ERROR_GOOGLE_ONLY }, 403, corsHeaders2) };
  }
  return { admin, user: data.user };
}, "requireGoogleUser");
var makeUsageId = /* @__PURE__ */ __name(() => {
...
var fetchTicketRow = /* @__PURE__ */ __name(async (admin, user) => {
  ...
  const { data: byEmail, error: emailError } = await admin.from("user_tickets").select("id, email, user_id, tickets").ilike("email", email).maybeSingle();
  if (emailError) {
...
var ensureTicketRow = /* @__PURE__ */ __name(async (admin, user) => {
  ...
  const { data: inserted, error: insertError } = await admin.from("user_tickets").insert({ email, user_id: user.id, tickets: SIGNUP_TICKET_GRANT }).select("id, email, user_id, tickets").maybeSingle();
...
var ensureTicketAvailable = /* @__PURE__ */ __name(async (admin, user, requiredTickets = 1, corsHeaders2 = {}) => {
  const email = user.email;
  if (!email) {
    return { response: jsonResponse3({ error: "Email not available." }, 400, corsHeaders2) };
  }
  ...
  if (!existing) {
    return { response: jsonResponse3({ error: "No tickets available." }, 402, corsHeaders2) };
  }
  if (!existing.user_id) {
    await admin.from("user_tickets").update({ user_id: user.id }).eq("id", existing.id);
  }
  if (existing.tickets < requiredTickets) {
    return { response: jsonResponse3({ error: "No tickets remaining." }, 402, corsHeaders2) };
  }
  return { existing };
}, "ensureTicketAvailable");
var consumeTicket = /* @__PURE__ */ __name(async (admin, user, metadata, usageId, ticketCost = 1, corsHeaders2 = {}) => {
  ...
  if (rpcError) {
    ...
    if (message.includes("INVALID")) {
      return { response: jsonResponse3({ error: "Invalid ticket request." }, 400, corsHeaders2) };
    }
    return { response: jsonResponse3({ error: INTERNAL_SERVER_ERROR_MESSAGE }, 500, corsHeaders2) };
  }
  const result = Array.isArray(rpcData) ? rpcData[0] : rpcData;
...
var refundTicket = /* @__PURE__ */ __name(async (admin, user, metadata, usageId, ticketCost = 1, corsHeaders2 = {}) => {
  const refundAmount = Math.max(1, Math.floor(ticketCost));
  const email = user.email;
  if (!email || !usageId) {
    return { skipped: true };
  }
  const { data: chargeEvent, error: chargeError } = await admin.from("ticket_events").select("usage_id, user_id, email, delta").eq("usage_id", usageId).maybeSingle();
  if (chargeError) {
    return { response: jsonResponse3({ error: INTERNAL_SERVER_ERROR_MESSAGE }, 500, corsHeaders2) };
  }
  if (!chargeEvent) {
  ...
  const chargeOwner = {
    user_id: chargeEvent.user_id ? String(chargeEvent.user_id) : null,
    email: chargeEvent.email ? String(chargeEvent.email) : null
  };
...
var resolveUsageTicketMeta = /* @__PURE__ */ __name((event) => {
  if (!event) {
    return { ticketCost: null, seconds: null };
  }
  let ticketCost = null;
...
var sanitizeText = /* @__PURE__ */ __name((value) => {
  if (!value) return value;
  if (value.startsWith("data:")) return value;
  return value.replace(HIDDEN_MODEL_NAME_PATTERN, GENERIC_MODEL_LABEL).replace(HIDDEN_MODEL_FILE_PATTERN, `${GENERIC_MODEL_LABEL}.bin`);
}, "sanitizeText");
var sanitizePayload = /* @__PURE__ */ __name((value) => {
  if (typeof value === "string") {
    if (value.length > 2e3 && value.startsWith("data:")) return value;
...
var ensureBase64Input = /* @__PURE__ */ __name((label, value, maxBytes = MAX_IMAGE_BYTES) => {
...
var parseLoraPackSelections = /* @__PURE__ */ __name((value) => {
  if (value === void 0 || value === null || value === "") {
    return { selections: [], error: null };
  }
  if (!Array.isArray(value)) {
    return { selections: [], error: "loras must be an array." };
  }
  ...
  for (const item of value) {
    const idRaw = typeof item === "string" ? item : item && typeof item === "object" && !Array.isArray(item) ? item.id : null;
    if (typeof idRaw !== "string" || !idRaw) {
      return { selections: [], error: "Invalid lora item." };
    }
    if (!(idRaw in LORA_PACK_OPTIONS)) {
      return { selections: [], error: "Unknown lora id." };
    }
    const strengthRaw = item && typeof item === "object" && !Array.isArray(item) ? item.strength : void 0;
    const strength = strengthRaw === void 0 || strengthRaw === null || strengthRaw === "" ? DEFAULT_LORA_PACK_STRENGTH : Number(strengthRaw);
...
}, "parseLoraPackSelections");
var withLoraPackTriggers = /* @__PURE__ */ __name((prompt, selections) => {
  ...
  return nextPrompt;
}, "withLoraPackTriggers");
var applyLoraPackStack = /* @__PURE__ */ __name((workflow, selections) => {
  const highUnetNodeIds = [];
  const lowUnetNodeIds = [];
  ...
  const existingLoraNodeIds = Object.entries(workflow).filter(([, node]) => node?.class_type === "LoraLoaderModelOnly").map(([id]) => id);
  for (const id of existingLoraNodeIds) {
    delete workflow[id];
  }
  let nextId = Math.max(
    ...Object.keys(workflow).map((id) => Number(id)).filter((id) => Number.isFinite(id))
  ) + 1;
  if (!Number.isFinite(nextId)) nextId = 1e3;
  const createChain = /* @__PURE__ */ __name((baseNodeId, side) => {
    let currentNodeId = baseNodeId;
    const loadedLoras = /* @__PURE__ */ new Set();
  ...
  for (const node of Object.values(workflow)) {
    if (node?.class_type !== "ModelSamplingSD3") continue;
    ...
    if (Array.isArray(modelRef) && modelRef.length > 0) {
      ...
      if (sourceNodeId === highBaseNodeId || sourceNodeId === highFinalNodeId) {
        ;
        node.inputs.model = [highFinalNodeId, 0];
      } else if (sourceNodeId === lowBaseNodeId || sourceNodeId === lowFinalNodeId) {
        ;
        node.inputs.model = [lowFinalNodeId, 0];
      }
    }
    ;
    node.inputs.shift = LORA_PACK_SHIFT;
  }
}, "applyLoraPackStack");
...
var onRequestGet = /* @__PURE__ */ __name(async ({ request, env }) => {
  ...
  if (!isFreeGuestMode) {
    const auth = await requireGoogleUser2(request, env, corsHeaders2);
    if ("response" in auth) {
      return auth.response;
    }
    authContext = auth;
  ...
  if (!id) {
    return jsonResponse3({ error: ERROR_ID_REQUIRED }, 400, corsHeaders2);
  }
  const modeParam = String(url.searchParams.get("mode") ?? "").toLowerCase();
  if (modeParam === "t2v") {
    return jsonResponse3({ error: 'mode "t2v" is no longer supported.' }, 400, corsHeaders2);
  }
  const statusMode = modeParam === "animate" ? "animate" : "i2v";
  if (i2vVariant === "dasiwa" && statusMode !== "i2v") {
    return jsonResponse3({ error: 'wan-dasiwa supports mode "i2v" only.' }, 400, corsHeaders2);
  }
  if (isLoraPack && statusMode !== "i2v") {
    return jsonResponse3({ error: 'wan-lora-pack supports mode "i2v" only.' }, 400, corsHeaders2);
  }
  if (!env.RUNPOD_API_KEY) {
    return jsonResponse3({ error: "RUNPOD_API_KEY is not set." }, 500, corsHeaders2);
  }
  const endpoint = resolveEndpoint(env, request, statusMode, i2vVariant);
  if (!endpoint) {
    return jsonResponse3(
      {
        error: "WAN endpoint is not set. Configure RUNPOD_WAN_DASIWA_ENDPOINT_URL or RUNPOD_WAN_RAPID_FASTMOVE_ENDPOINT_URL for i2v and RUNPOD_WAN_ENDPOINT_URL for animate."
      },
      500,
      corsHeaders2
    );
  ...
  let usageTicketMeta = null;
  if (!isFreeGuestMode && authContext) {
    const usageEventResult = await requireOwnedUsageChargeEvent(authContext.admin, authContext.user, usageId, corsHeaders2);
  ...
  const requestedDurationOption = requestedSeconds !== null && Number.isFinite(requestedSeconds) ? resolveVideoDurationOption(requestedSeconds, i2vVariant, isLoraPack) ?? getDefaultVideoDurationOption(i2vVariant, isLoraPack) : getDefaultVideoDurationOption(i2vVariant, isLoraPack);
  const statusDurationOption = statusMode === "animate" ? null : resolveVideoDurationOption(usageTicketMeta?.seconds ?? requestedDurationOption.seconds, i2vVariant, isLoraPack) ?? requestedDurationOption;
  const ticketCost = statusMode === "animate" ? DEFAULT_VIDEO_TICKET_COST : usageTicketMeta?.ticketCost ?? statusDurationOption.ticketCost;
  let upstream;
  try {
    ...
    return jsonResponse3({ error: INTERNAL_SERVER_ERROR_MESSAGE }, 502, corsHeaders2);
  }
  ...
  if (!isFreeGuestMode && authContext && payload && shouldConsumeTicket(payload)) {
    ...
    const result = await consumeTicket(authContext.admin, authContext.user, ticketMeta, usageId, ticketCost, corsHeaders2);
  ...
  if (!isFreeGuestMode && authContext && payload && (isFailureStatus(payload) || hasOutputError(payload))) {
    ...
    const refundResult = await refundTicket(authContext.admin, authContext.user, refundMeta, usageId, ticketCost, corsHeaders2);
  ...
  if (ticketsLeft !== null && payload && typeof payload === "object" && !Array.isArray(payload)) {
    ...
    return jsonResponse3(sanitizePayload(payload), upstream.status, corsHeaders2);
  }
  if (payload && typeof payload === "object") {
    return jsonResponse3(sanitizePayload(payload), upstream.status, corsHeaders2);
  }
  if (!upstream.ok) {
    return jsonResponse3({ error: INTERNAL_SERVER_ERROR_MESSAGE }, upstream.status, corsHeaders2);
  }
...
var onRequestPost3 = /* @__PURE__ */ __name(async ({ request, env }) => {
  ...
  const payload = await request.json().catch(() => null);
  if (!payload) {
    return jsonResponse3({ error: "Invalid request body." }, 400, corsHeaders2);
  }
  const input = payload.input ?? payload;
  if (input?.workflow) {
    return jsonResponse3({ error: "workflow overrides are not allowed." }, 400, corsHeaders2);
  }
  ...
  if (mode !== "i2v" && mode !== "animate") {
    return jsonResponse3({ error: 'mode must be "i2v" or "animate".' }, 400, corsHeaders2);
  }
  ...
  const durationResult = isAnimate ? { option: null, error: null } : resolveVideoDurationFromInput(input?.seconds, i2vVariant, isLoraPack);
  if (!isAnimate && (durationResult.error || !durationResult.option)) {
    return jsonResponse3({ error: durationResult.error ?? "Invalid seconds value." }, 400, corsHeaders2);
  }
  ...
  if (!isAnimate && !imageValue) {
    return jsonResponse3({ error: ERROR_I2V_IMAGE_REQUIRED }, 400, corsHeaders2);
  }
  const videoValue = input?.video_base64 ?? input?.video ?? input?.video_url;
  if (isAnimate && !videoValue) {
    return jsonResponse3({ error: ERROR_ANIMATE_VIDEO_REQUIRED }, 400, corsHeaders2);
  }
  let imageBase64 = "";
  let videoBase64 = "";
  try {
  ...
  if (!isAnimate && !imageBase64) {
    return jsonResponse3({ error: "image is empty." }, 400, corsHeaders2);
  }
  if (isAnimate && !videoBase64) {
    return jsonResponse3({ error: "video is empty." }, 400, corsHeaders2);
  }
  if (!isAnimate) {
    try {
      if (await isUnderageImage(imageBase64, env)) {
        return jsonResponse3({ error: UNDERAGE_BLOCK_MESSAGE }, 400, corsHeaders2);
      }
    } catch (error) {
      return jsonResponse3(
  ...
  const loraSelectionResult = isLoraPack ? parseLoraPackSelections(input?.loras) : { selections: [], error: null };
  if (loraSelectionResult.error) {
    return jsonResponse3({ error: loraSelectionResult.error }, 400, corsHeaders2);
  }
  ...
  const numFrames = isAnimate ? Math.max(1, Math.floor(Number(input?.num_frames ?? FIXED_ANIMATE_FRAMES))) : videoDurationOption.frames;
  ...
  if (prompt.length > MAX_PROMPT_LENGTH) {
    return jsonResponse3({ error: "Prompt is too long." }, 400, corsHeaders2);
  }
  if (negativePrompt.length > MAX_NEGATIVE_PROMPT_LENGTH) {
    return jsonResponse3({ error: "Negative prompt is too long." }, 400, corsHeaders2);
  }
  if (!Number.isFinite(cfg) || cfg < MIN_CFG || cfg > MAX_CFG) {
    return jsonResponse3({ error: `cfg must be between ${MIN_CFG} and ${MAX_CFG}.` }, 400, corsHeaders2);
  }
  ...
  if (!isFreeGuestMode && authContext) {
    const ticketCheck = await ensureTicketAvailable(authContext.admin, authContext.user, ticketCost, corsHeaders2);
  ...
  if (!workflow || Object.keys(workflow).length === 0) {
    return jsonResponse3({ error: "wan workflow is empty. Export a ComfyUI API workflow." }, 500, corsHeaders2);
  }
  if (isLoraPackRoute(request)) {
    applyLoraPackStack(workflow, activeLoraSelections);
  }
  const nodeMap = await getNodeMap(generationMode).catch(() => null);
  const hasNodeMap = nodeMap && Object.keys(nodeMap).length > 0;
  if (!hasNodeMap) {
    return jsonResponse3({ error: "wan node map is empty." }, 500, corsHeaders2);
  }
  const nodeValues = {
    image: imageBase64 ? imageName : void 0,
    video: videoBase64 ? videoName : void 0,
    prompt,
  ...
  const shouldCharge = upstream.ok && Boolean(jobId) && !isFailureStatus(upstreamPayload) && !hasOutputError(upstreamPayload);
  if (!isFreeGuestMode && authContext && shouldCharge && jobId) {
    ...
    const result = await consumeTicket(authContext.admin, authContext.user, ticketMetaWithJob, usageId, ticketCost, corsHeaders2);
    if ("response" in result) {
      return result.response;
    }
    const nextTickets = Number(result.ticketsLeft);
    ...
    const jobId2 = extractJobId(upstreamPayload);
  ...
  if (ticketsLeft !== null && upstreamPayload && typeof upstreamPayload === "object" && !Array.isArray(upstreamPayload)) {
    ...
    return jsonResponse3(sanitizePayload(upstreamPayload), upstream.status, corsHeaders2);
  }
...
var BONUS_COOLDOWN_HOURS = 12;
var BONUS_COOLDOWN_MS = BONUS_COOLDOWN_HOURS * 60 * 60 * 1e3;
var BONUS_AMOUNT = 1;
var DAILY_BONUS_REASONS = ["daily_bonus", "daily_bonus_claim"];
var INTERNAL_SERVER_ERROR_MESSAGE2 = "\u30B5\u30FC\u30D0\u30FC\u5185\u90E8\u30A8\u30E9\u30FC\u304C\u767A\u751F\u3057\u307E\u3057\u305F\u3002\u6642\u9593\u3092\u304A\u3044\u3066\u518D\u5EA6\u304A\u8A66\u3057\u304F\u3060\u3055\u3044\u3002";
var ERROR_LOGIN_REQUIRED2 = "\u30ED\u30B0\u30A4\u30F3\u304C\u5FC5\u8981\u3067\u3059\u3002";
var ERROR_AUTH_FAILED2 = "\u8A8D\u8A3C\u306B\u5931\u6557\u3057\u307E\u3057\u305F\u3002";
var ERROR_GOOGLE_ONLY2 = "Google\u30ED\u30B0\u30A4\u30F3\u306E\u307F\u5BFE\u5FDC\u3057\u3066\u3044\u307E\u3059\u3002";
var ERROR_SUPABASE_NOT_SET2 = "Supabase\u74B0\u5883\u5909\u6570\u304C\u8A2D\u5B9A\u3055\u308C\u3066\u3044\u307E\u305B\u3093\u3002";
...
var getSupabaseUserClient = /* @__PURE__ */ __name((env, accessToken) => {
  const url = env.SUPABASE_URL || env.VITE_SUPABASE_URL;
...
var isGoogleUser3 = /* @__PURE__ */ __name((user) => {
  if (user.app_metadata?.provider === "google") return true;
  if (Array.isArray(user.identities)) {
    return user.identities.some((identity) => identity.provider === "google");
  }
...
var requireGoogleUser3 = /* @__PURE__ */ __name(async (request, env, corsHeaders2) => {
  const token = extractBearerToken3(request);
  if (!token) {
    return { response: jsonResponse4({ error: ERROR_LOGIN_REQUIRED2 }, 401, corsHeaders2) };
  }
  const admin = getSupabaseAdmin4(env);
  if (!admin) {
    return { response: jsonResponse4({ error: ERROR_SUPABASE_NOT_SET2 }, 500, corsHeaders2) };
  }
  const { data, error } = await getSupabaseUserWithRetry(admin, token);
  if (error || !data?.user) {
    return { response: jsonResponse4({ error: ERROR_AUTH_FAILED2 }, 401, corsHeaders2) };
  }
  if (!isGoogleUser3(data.user)) {
    return { response: jsonResponse4({ error: ERROR_GOOGLE_ONLY2 }, 403, corsHeaders2) };
  }
  return { admin, user: data.user, token };
}, "requireGoogleUser");
var fetchLatestClaimAt = /* @__PURE__ */ __name(async (admin, userId, email) => {
  const byUser = await admin.from("ticket_events").select("created_at").eq("user_id", userId).in("reason", DAILY_BONUS_REASONS).order("created_at", { ascending: false }).limit(1).maybeSingle();
  if (byUser.error) return { error: byUser.error, createdAt: null };
  if (byUser.data?.created_at) return { error: null, createdAt: String(byUser.data.created_at) };
  if (!email) return { error: null, createdAt: null };
  const byEmail = await admin.from("ticket_events").select("created_at").eq("email", email).in("reason", DAILY_BONUS_REASONS).order("created_at", { ascending: false }).limit(1).maybeSingle();
  if (byEmail.error) return { error: byEmail.error, createdAt: null };
  return { error: null, createdAt: byEmail.data?.created_at ? String(byEmail.data.created_at) : null };
}, "fetchLatestClaimAt");
var parseTimeMs = /* @__PURE__ */ __name((value) => {
  if (!value) return null;
  const ms = new Date(value).getTime();
  return Number.isFinite(ms) ? ms : null;
}, "parseTimeMs");
var buildBonusStatus = /* @__PURE__ */ __name((latestClaimAt, userCreatedAt) => {
  const now = Date.now();
  ...
  return {
    ...
    remainingSeconds: Math.ceil(diff / 1e3)
  };
}, "buildBonusStatus");
var getDailyBonusStatus = /* @__PURE__ */ __name(async (admin, user) => {
  const email = user.email ?? "";
  ...
  if (latest.error) {
    return { error: latest.error, status: null };
  }
...
var onRequestGet2 = /* @__PURE__ */ __name(async ({ request, env }) => {
  ...
  const auth = await requireGoogleUser3(request, env, corsHeaders2);
  if ("response" in auth) {
    return auth.response;
  }
  const statusResult = await getDailyBonusStatus(auth.admin, auth.user);
  if (statusResult.error || !statusResult.status) {
    return jsonResponse4({ error: INTERNAL_SERVER_ERROR_MESSAGE2 }, 500, corsHeaders2);
  }
...
var onRequestPost4 = /* @__PURE__ */ __name(async ({ request, env }) => {
  ...
  if (!userClient) {
    return jsonResponse4({ error: ERROR_SUPABASE_NOT_SET2 }, 500, corsHeaders2);
  }
...
var SIGNUP_TICKET_GRANT2 = 3;
var TICKET_COST = 1;
var MAX_IMAGE_BYTES2 = 10 * 1024 * 1024;
var MAX_REFERENCE_IMAGES = 1;
var MAX_PROMPT_LENGTH2 = 1e3;
var MAX_NEGATIVE_PROMPT_LENGTH2 = 1e3;
var DEFAULT_ENDPOINT = "https://api.runpod.ai/v2/waeacog3y0gk1e";
var DEFAULT_CFG = 1;
var FIXED_STEPS2 = 4;
var FIXED_MEGAPIXELS = 1;
var DEFAULT_SAMPLER = "euler";
var FILENAME_PREFIX = "orca-image-edit";
var ANGLE_LORA_NAME = "flux-multi-angles-v2-72poses-comfy.safetensors";
var ANGLE_LORA_STRENGTH = 0.7;
var ANGLE_PROMPTS = {
  none: "",
...
var INTERNAL_SERVER_ERROR_MESSAGE3 = "Server processing failed. Please try again later.";
var ERROR_LOGIN_REQUIRED3 = "Login is required.";
var ERROR_AUTH_FAILED3 = "Authentication failed.";
var ERROR_GOOGLE_ONLY3 = "Google login is required.";
var ERROR_SUPABASE_NOT_SET3 = "Supabase server setting is missing.";
var ERROR_EMAIL_MISSING = "Email address is missing.";
var ERROR_NO_TICKETS = "Not enough tickets.";
var ERROR_INVALID_TICKET_REQUEST = "Invalid ticket operation.";
var ERROR_ID_REQUIRED2 = "id is required.";
var ERROR_USAGE_ID_REQUIRED = "usage_id is required.";
var ERROR_JOB_NOT_FOUND2 = "Job not found.";
var HIDDEN_MODEL_FILE_PATTERN2 = /[A-Za-z0-9_./%\\\-\s]+?\.(safetensors|gguf|onnx|pt|pth|ckpt|bin)/gi;
var HIDDEN_MODEL_NAME_PATTERN2 = /qwen|flux|klein|lora|comfy|runpod|image[-_\s]?edit|gumix/gi;
var GENERIC_MODEL_LABEL2 = "image edit model";
var UNSAFE_PAYLOAD_KEYS = /* @__PURE__ */ new Set([
  "data",
  "image",
  "images",
  "base64",
...
var sanitizeUserText = /* @__PURE__ */ __name((value) => value.replace(HIDDEN_MODEL_NAME_PATTERN2, GENERIC_MODEL_LABEL2).replace(HIDDEN_MODEL_FILE_PATTERN2, `${GENERIC_MODEL_LABEL2}.bin`), "sanitizeUserText");
var sanitizeJsonPayload = /* @__PURE__ */ __name((value, key = "") => {
  if (typeof value === "string") {
...
var resolveEndpoint2 = /* @__PURE__ */ __name((env) => normalizeEndpoint(env.RUNPOD_SUPER_IMAGE_EDIT_ENDPOINT_URL) || DEFAULT_ENDPOINT, "resolveEndpoint");
var resolveRunpodApiKey = /* @__PURE__ */ __name((env) => (env.RUNPOD_SUPER_IMAGE_EDIT_API_KEY || env.RUNPOD_API_KEY || "").trim(), "resolveRunpodApiKey");
var isFile = /* @__PURE__ */ __name((value) => value instanceof File, "isFile");
var normalizeEmail2 = /* @__PURE__ */ __name((value) => (value ?? "").trim().toLowerCase(), "normalizeEmail");
var isGoogleUser4 = /* @__PURE__ */ __name((user) => {
  if (user.app_metadata?.provider === "google") return true;
...
var requireGoogleUser4 = /* @__PURE__ */ __name(async (request, env, corsHeaders2) => {
  const token = extractBearerToken4(request);
  if (!token) return { response: jsonResponse5({ error: ERROR_LOGIN_REQUIRED3 }, 401, corsHeaders2) };
  const admin = getSupabaseAdmin5(env);
  if (!admin) return { response: jsonResponse5({ error: ERROR_SUPABASE_NOT_SET3 }, 500, corsHeaders2) };
  const { data, error } = await getSupabaseUserWithRetry(admin, token);
  if (error || !data?.user) return { response: jsonResponse5({ error: ERROR_AUTH_FAILED3 }, 401, corsHeaders2) };
  if (!isGoogleUser4(data.user)) return { response: jsonResponse5({ error: ERROR_GOOGLE_ONLY3 }, 403, corsHeaders2) };
  return { admin, user: data.user };
}, "requireGoogleUser");
...
var parseTicketMetadata2 = /* @__PURE__ */ __name((value) => {
  if (!value || typeof value !== "object" || Array.isArray(value)) return null;
  return value;
}, "parseTicketMetadata");
var isUsageOwnedByUser2 = /* @__PURE__ */ __name((event, user) => {
  if (event.user_id && event.user_id === user.id) return true;
  const userEmail = normalizeEmail2(user.email ?? "");
  return Boolean(userEmail && normalizeEmail2(event.email) === userEmail);
}, "isUsageOwnedByUser");
var fetchUsageEvent2 = /* @__PURE__ */ __name(async (admin, usageId) => {
  const { data, error } = await admin.from("ticket_events").select("usage_id, user_id, email, delta, metadata").eq("usage_id", usageId).maybeSingle();
  if (error || !data) return { event: null, error };
  return {
    event: {
      usage_id: String(data.usage_id),
      user_id: data.user_id ? String(data.user_id) : null,
      email: data.email ? String(data.email) : null,
      delta: Number.isFinite(Number(data.delta)) ? Number(data.delta) : null,
      metadata: parseTicketMetadata2(data.metadata)
    },
    error: null
  };
}, "fetchUsageEvent");
...
var consumeTicket2 = /* @__PURE__ */ __name(async (admin, user, metadata, usageId, corsHeaders2) => {
  if (!user.email) return { response: jsonResponse5({ error: ERROR_EMAIL_MISSING }, 400, corsHeaders2) };
  const { data: existing, error } = await ensureTicketRow2(admin, user);
  if (error) return { response: jsonResponse5({ error: INTERNAL_SERVER_ERROR_MESSAGE3 }, 500, corsHeaders2) };
  if (!existing) return { response: jsonResponse5({ error: ERROR_NO_TICKETS }, 402, corsHeaders2) };
  if (!existing.user_id) await admin.from("user_tickets").update({ user_id: user.id }).eq("id", existing.id);
  if (Number(existing.tickets) < TICKET_COST) return { response: jsonResponse5({ error: ERROR_NO_TICKETS }, 402, corsHeaders2) };
  const { data: rpcData, error: rpcError } = await admin.rpc("consume_tickets", {
    p_ticket_id: existing.id,
    p_usage_id: usageId,
    p_cost: TICKET_COST,
  ...
  if (rpcError) {
    const message = rpcError.message ?? INTERNAL_SERVER_ERROR_MESSAGE3;
    if (message.includes("INSUFFICIENT_TICKETS")) return { response: jsonResponse5({ error: ERROR_NO_TICKETS }, 402, corsHeaders2) };
    if (message.includes("INVALID")) return { response: jsonResponse5({ error: ERROR_INVALID_TICKET_REQUEST }, 400, corsHeaders2) };
    return { response: jsonResponse5({ error: INTERNAL_SERVER_ERROR_MESSAGE3 }, 500, corsHeaders2) };
  }
  const result = Array.isArray(rpcData) ? rpcData[0] : rpcData;
  const ticketsLeft = Number(result?.tickets_left);
  return { ticketsLeft: Number.isFinite(ticketsLeft) ? ticketsLeft : void 0 };
}, "consumeTicket");
var refundTicket2 = /* @__PURE__ */ __name(async (admin, user, metadata, usageId, corsHeaders2) => {
  if (!user.email || !usageId) return { skipped: true };
  const { data: chargeEvent, error: chargeError } = await admin.from("ticket_events").select("usage_id, user_id, email, delta").eq("usage_id", usageId).maybeSingle();
  if (chargeError) return { response: jsonResponse5({ error: INTERNAL_SERVER_ERROR_MESSAGE3 }, 500, corsHeaders2) };
  ...
  if (!Number.isFinite(chargeDelta) || chargeDelta >= 0 || !isUsageOwnedByUser2(chargeOwner, user)) return { skipped: true };
  const refundUsageId = `${usageId}:refund`;
  const { data: existingRefund, error: refundCheckError } = await admin.from("ticket_events").select("usage_id").eq("usage_id", refundUsageId).maybeSingle();
  if (refundCheckError) return { response: jsonResponse5({ error: INTERNAL_SERVER_ERROR_MESSAGE3 }, 500, corsHeaders2) };
  if (existingRefund) return { alreadyRefunded: true };
  const { data: ticketRow, error } = await ensureTicketRow2(admin, user);
  if (error) return { response: jsonResponse5({ error: INTERNAL_SERVER_ERROR_MESSAGE3 }, 500, corsHeaders2) };
  ...
  if (rpcError) return { response: jsonResponse5({ error: INTERNAL_SERVER_ERROR_MESSAGE3 }, 500, corsHeaders2) };
  const result = Array.isArray(rpcData) ? rpcData[0] : rpcData;
  const ticketsLeft = Number(result?.tickets_left);
  return { ticketsLeft: Number.isFinite(ticketsLeft) ? ticketsLeft : void 0 };
}, "refundTicket");
var requireOwnedUsageChargeEvent2 = /* @__PURE__ */ __name(async (admin, user, usageId, corsHeaders2) => {
  const { event, error } = await fetchUsageEvent2(admin, usageId);
  if (error) return { response: jsonResponse5({ error: INTERNAL_SERVER_ERROR_MESSAGE3 }, 500, corsHeaders2) };
  if (!event || !isUsageOwnedByUser2(event, user) || Number(event.delta) >= 0) {
    return { response: jsonResponse5({ error: ERROR_JOB_NOT_FOUND2 }, 404, corsHeaders2) };
  }
  return { event };
}, "requireOwnedUsageChargeEvent");
var bindUsageToJob = /* @__PURE__ */ __name(async (admin, user, usageId, jobId) => {
  if (!jobId) return;
  const usageEvent = await fetchUsageEvent2(admin, usageId);
  if (usageEvent.error || !usageEvent.event || !isUsageOwnedByUser2(usageEvent.event, user)) return;
...
var bytesToBase64 = /* @__PURE__ */ __name((buffer) => {
...
var fileToBase64 = /* @__PURE__ */ __name(async (file) => bytesToBase64(await file.arrayBuffer()), "fileToBase64");
var safeFileName = /* @__PURE__ */ __name((value, fallback) => {
  const cleaned = value.replace(/\\/g, "/").split("/").pop()?.replace(/[^A-Za-z0-9._-]+/g, "_").replace(/^_+|_+$/g, "");
  return cleaned || fallback;
}, "safeFileName");
var readJsonResponse = /* @__PURE__ */ __name(async (response) => {
...
var extractJobId2 = /* @__PURE__ */ __name((payload) => payload?.id || payload?.jobId || payload?.job_id || payload?.output?.id || payload?.result?.id, "extractJobId");
var extractStatus = /* @__PURE__ */ __name((payload) => String(
  payload?.status ?? payload?.state ?? payload?.output?.status ?? payload?.result?.status ?? payload?.output?.output?.status ?? payload?.result?.output?.status ?? ""
), "extractStatus");
var hasImageString = /* @__PURE__ */ __name((value) => {
...
var normalizeImageString = /* @__PURE__ */ __name((value) => {
...
var extractImageFromItem = /* @__PURE__ */ __name((value) => {
  const direct = normalizeImageString(value);
  if (direct) return direct;
  if (!value || typeof value !== "object") return null;
  const item = value;
  return normalizeImageString(
    item.image ?? item.data ?? item.base64 ?? item.content ?? item.b64 ?? item.image_base64 ?? item.output_image_base64 ?? item.url ?? item.image_url ?? item.file_url ?? item.download_url ?? item.output ?? item.result
  );
}, "extractImageFromItem");
var extractImageList = /* @__PURE__ */ __name((payload) => {
  if (!payload) return [];
  const roots = [
    payload,
    payload?.output,
  ...
  for (const root of roots) {
    if (Array.isArray(root)) {
      const images = root.map(extractImageFromItem).filter(Boolean);
    ...
    for (const list of lists) {
      ...
      const images = list.map(extractImageFromItem).filter(Boolean);
      if (images.length) return images;
    }
  }
  return [];
}, "extractImageList");
var hasAssetItem = /* @__PURE__ */ __name((value) => {
...
var extractOutputError = /* @__PURE__ */ __name((payload) => {
  const value = payload?.error ?? payload?.message ?? payload?.detail ?? payload?.output?.error ?? payload?.result?.error ?? payload?.output?.output?.error ?? payload?.result?.output?.error;
  if (!value) return "";
...
var onRequestOptions5 = /* @__PURE__ */ __name(async ({ request, env }) => {
  ...
  if (isCorsBlocked(request, env)) return new Response(null, { status: 403, headers: corsHeaders2 });
  return new Response(null, { headers: corsHeaders2 });
}, "onRequestOptions");
var onRequestGet3 = /* @__PURE__ */ __name(async ({ request, env }) => {
  const corsHeaders2 = buildCorsHeaders(request, env, corsMethods4);
  if (isCorsBlocked(request, env)) return new Response(null, { status: 403, headers: corsHeaders2 });
  const auth = await requireGoogleUser4(request, env, corsHeaders2);
  if ("response" in auth) return auth.response;
  const url = new URL(request.url);
  const id = url.searchParams.get("id")?.trim();
  const usageId = url.searchParams.get("usage_id")?.trim() || "";
  if (!id) return jsonResponse5({ error: ERROR_ID_REQUIRED2 }, 400, corsHeaders2);
  if (!usageId) return jsonResponse5({ error: ERROR_USAGE_ID_REQUIRED }, 400, corsHeaders2);
  const usageEventResult = await requireOwnedUsageChargeEvent2(auth.admin, auth.user, usageId, corsHeaders2);
  ...
  if (usageJobId && usageJobId !== id) return jsonResponse5({ error: ERROR_JOB_NOT_FOUND2 }, 404, corsHeaders2);
  ...
  if (!runpodApiKey || !endpoint) return jsonResponse5({ error: INTERNAL_SERVER_ERROR_MESSAGE3 }, 500, corsHeaders2);
  let upstream;
  try {
    upstream = await fetch(`${endpoint}/status/${encodeURIComponent(id)}`, {
    ...
    return jsonResponse5({ error: INTERNAL_SERVER_ERROR_MESSAGE3 }, 502, corsHeaders2);
  }
  ...
  let ticketsLeft;
  if (isFailureStatus2(data) || hasOutputError2(data)) {
    const refundResult = await refundTicket2(
      auth.admin,
      auth.user,
  ...
  if (ticketsLeft !== void 0) data.ticketsLeft = ticketsLeft;
  return jsonResponse5(sanitizeJsonPayload(data), upstream.status, corsHeaders2);
}, "onRequestGet");
var onRequestPost5 = /* @__PURE__ */ __name(async ({ request, env }) => {
  ...
  if (!formData) return jsonResponse5({ error: "multipart/form-data request body is required." }, 400, corsHeaders2);
  ...
  if (!isFile(imageFile) || imageFile.size === 0) return jsonResponse5({ error: "Image file is required." }, 400, corsHeaders2);
  if (imageFile.size > MAX_IMAGE_BYTES2) return jsonResponse5({ error: "Image file is too large." }, 400, corsHeaders2);
  if (referenceFiles.length > MAX_REFERENCE_IMAGES) {
    return jsonResponse5({ error: `Reference images must be ${MAX_REFERENCE_IMAGES} or fewer.` }, 400, corsHeaders2);
  }
  for (const file of referenceFiles) {
    if (file.size > MAX_IMAGE_BYTES2) return jsonResponse5({ error: "Reference image file is too large." }, 400, corsHeaders2);
  }
  if (!prompt && !anglePrompt) return jsonResponse5({ error: "Prompt or angle preset is required." }, 400, corsHeaders2);
  if (prompt.length > MAX_PROMPT_LENGTH2) return jsonResponse5({ error: "Prompt is too long." }, 400, corsHeaders2);
  if (negativePrompt.length > MAX_NEGATIVE_PROMPT_LENGTH2) {
    return jsonResponse5({ error: "Negative prompt is too long." }, 400, corsHeaders2);
  }
  const finalPrompt = [anglePrompt, prompt].filter(Boolean).join(", ");
  const loras = anglePrompt && angleId !== "none" ? [{ lora_name: ANGLE_LORA_NAME, strength_model: ANGLE_LORA_STRENGTH, strength_clip: 0 }] : [];
  const usageId = `image_edit:${makeUsageId2()}`;
  let ticketsLeft;
  const ticketMeta = {
    source: "image_edit",
    prompt_length: prompt.length,
  ...
  const ticketCharge = await consumeTicket2(auth.admin, auth.user, ticketMeta, usageId, corsHeaders2);
  ...
  try {
    ...
    const refundResult = await refundTicket2(auth.admin, auth.user, { ...ticketMeta, reason: "network_error" }, usageId, corsHeaders2);
    if ("response" in refundResult) return refundResult.response;
    if ("ticketsLeft" in refundResult) ticketsLeft = refundResult.ticketsLeft;
    return jsonResponse5({ error: INTERNAL_SERVER_ERROR_MESSAGE3, usage_id: usageId, ticketsLeft }, 502, corsHeaders2);
  }
  const { raw, data } = await readJsonResponse(upstream);
  if (!data || typeof data !== "object" || Array.isArray(data)) {
    const refundResult = await refundTicket2(auth.admin, auth.user, { ...ticketMeta, reason: "parse_error" }, usageId, corsHeaders2);
    if ("response" in refundResult) return refundResult.response;
    if ("ticketsLeft" in refundResult) ticketsLeft = refundResult.ticketsLeft;
    return jsonResponse5({ error: INTERNAL_SERVER_ERROR_MESSAGE3, usage_id: usageId, ticketsLeft, raw: raw.slice(0, 256) }, 502, corsHeaders2);
  }
  const jobId = extractJobId2(data);
  if (jobId) await bindUsageToJob(auth.admin, auth.user, usageId, String(jobId));
  const isFailure = !upstream.ok || isFailureStatus2(data) || hasOutputError2(data);
  if (isFailure) {
    const refundResult = await refundTicket2(
      auth.admin,
      auth.user,
  ...
  if (!jobId && !hasAssets2(data)) {
    const refundResult = await refundTicket2(auth.admin, auth.user, { ...ticketMeta, reason: "missing_job_id" }, usageId, corsHeaders2);
    if ("response" in refundResult) return refundResult.response;
    if ("ticketsLeft" in refundResult) ticketsLeft = refundResult.ticketsLeft;
    return jsonResponse5({ error: INTERNAL_SERVER_ERROR_MESSAGE3, usage_id: usageId, ticketsLeft }, 502, corsHeaders2);
  }
...
var DEFAULT_MMAUDIO_ENDPOINT = "https://api.runpod.ai/v2/8kch616iovemh8";
var MMAUDIO_TARGET_FPS = 25;
var MAX_PROMPT_LENGTH3 = 12e3;
var MAX_VIDEO_BYTES2 = 30 * 1024 * 1024;
var PIPELINE_USAGE_ID_MAX_LENGTH = 128;
var PIPELINE_USAGE_ID_MAX_AGE_MS = 15 * 60 * 1e3;
var PIPELINE_USAGE_ID_PATTERN = /^media:(\d{13}):([A-Za-z0-9-]{16,96})$/;
var SIGNUP_TICKET_GRANT3 = 3;
var MMAUDIO_TICKET_COST = 1;
var ERR_SERVER_CONFIG = "\u30B5\u30FC\u30D0\u30FC\u8A2D\u5B9A\u304C\u4E0D\u8DB3\u3057\u3066\u3044\u307E\u3059\u3002";
var ERR_INVALID_BODY = "\u30EA\u30AF\u30A8\u30B9\u30C8\u5F62\u5F0F\u304C\u4E0D\u6B63\u3067\u3059\u3002";
var ERR_PROMPT_REQUIRED = "\u30C6\u30AD\u30B9\u30C8\u3092\u5165\u529B\u3057\u3066\u304F\u3060\u3055\u3044\u3002";
var ERR_PROMPT_TOO_LONG = `text is too long (max ${MAX_PROMPT_LENGTH3}).`;
var ERR_VIDEO_REQUIRED = "\u52D5\u753B\u30C7\u30FC\u30BF\u3092\u5165\u529B\u3057\u3066\u304F\u3060\u3055\u3044\u3002";
var ERR_VIDEO_TOO_LARGE = `video is too large (max ${MAX_VIDEO_BYTES2 / (1024 * 1024)}MB).`;
var ERR_REQUEST_FAILED = "\u97F3\u58F0\u6F14\u51FA\u306E\u30EA\u30AF\u30A8\u30B9\u30C8\u306B\u5931\u6557\u3057\u307E\u3057\u305F\u3002";
var ERR_STATUS_FAILED = "\u97F3\u58F0\u6F14\u51FA\u306E\u72B6\u614B\u78BA\u8A8D\u306B\u5931\u6557\u3057\u307E\u3057\u305F\u3002";
var ERR_MUX_VIDEO_REQUIRED = "\u7D50\u5408\u5143\u306E\u52D5\u753B\u30C7\u30FC\u30BF\u304C\u5FC5\u8981\u3067\u3059\u3002";
var ERR_MUX_AUDIO_VIDEO_REQUIRED = "\u97F3\u58F0\u4ED8\u304D\u52D5\u753B\u30C7\u30FC\u30BF\u304C\u5FC5\u8981\u3067\u3059\u3002";
var ERR_MUX_FAILED = "\u52D5\u753B\u3068\u97F3\u58F0\u306E\u7D50\u5408\u306B\u5931\u6557\u3057\u307E\u3057\u305F\u3002";
var jsonResponse6 = /* @__PURE__ */ __name((body, status = 200, headers = {}) => new Response(JSON.stringify(body), {
  status,
  headers: { ...headers, "Content-Type": "application/json", "Cache-Control": "no-store" }
}), "jsonResponse");
var resolveEndpoint3 = /* @__PURE__ */ __name(() => DEFAULT_MMAUDIO_ENDPOINT, "resolveEndpoint");
var parseJsonSafe = /* @__PURE__ */ __name(async (request) => {
  try {
...
var normalizePipelineUsageId = /* @__PURE__ */ __name((raw, enforceAge = false) => {
  if (typeof raw !== "string") return "";
  const value = raw.trim();
  if (!value) return "";
  if (value.length > PIPELINE_USAGE_ID_MAX_LENGTH) return "";
  const match2 = value.match(PIPELINE_USAGE_ID_PATTERN);
  if (!match2) return "";
  if (enforceAge) {
    const createdAtMs = Number(match2[1]);
    if (!Number.isFinite(createdAtMs)) return "";
    const ageMs = Math.abs(Date.now() - createdAtMs);
    if (ageMs > PIPELINE_USAGE_ID_MAX_AGE_MS) return "";
  }
  return value;
}, "normalizePipelineUsageId");
var isPipelineUsageId = /* @__PURE__ */ __name((value) => PIPELINE_USAGE_ID_PATTERN.test(value), "isPipelineUsageId");
...
var isGoogleUser5 = /* @__PURE__ */ __name((user) => {
  if (user.app_metadata?.provider === "google") return true;
  if (Array.isArray(user.identities)) {
    return user.identities.some((identity) => identity.provider === "google");
  }
...
var normalizeEmail3 = /* @__PURE__ */ __name((value) => (value ?? "").trim().toLowerCase(), "normalizeEmail");
...
var requireGoogleUser5 = /* @__PURE__ */ __name(async (request, env, corsHeaders2) => {
  ...
  if (!admin) {
    return { response: jsonResponse6({ error: "SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY is not set." }, 500, corsHeaders2) };
  }
  const { data, error } = await getSupabaseUserWithRetry(admin, token);
  if (error || !data?.user) {
    return { response: jsonResponse6({ error: "\u8A8D\u8A3C\u306B\u5931\u6557\u3057\u307E\u3057\u305F\u3002" }, 401, corsHeaders2) };
  }
  if (!isGoogleUser5(data.user)) {
    return { response: jsonResponse6({ error: "Google\u30ED\u30B0\u30A4\u30F3\u306E\u307F\u5229\u7528\u3067\u304D\u307E\u3059\u3002" }, 403, corsHeaders2) };
  }
  return { admin, user: data.user };
}, "requireGoogleUser");
...
var ensureTicketAvailable2 = /* @__PURE__ */ __name(async (admin, user, requiredTickets, corsHeaders2) => {
  const email = user.email;
  if (!email) {
    return { response: jsonResponse6({ error: "Email not available." }, 400, corsHeaders2) };
  }
  const { data: existing, error } = await ensureTicketRow3(admin, user);
  if (error) {
    return { response: jsonResponse6({ error: "\u30B5\u30FC\u30D0\u30FC\u5185\u90E8\u30A8\u30E9\u30FC\u304C\u767A\u751F\u3057\u307E\u3057\u305F\u3002" }, 500, corsHeaders2) };
  }
  if (!existing) {
    return { response: jsonResponse6({ error: "No tickets available." }, 402, corsHeaders2) };
  }
  if (!existing.user_id) {
    await admin.from("user_tickets").update({ user_id: user.id }).eq("id", existing.id);
  }
  if (existing.tickets < requiredTickets) {
    return { response: jsonResponse6({ error: "No tickets remaining." }, 402, corsHeaders2) };
  }
  return { existing };
}, "ensureTicketAvailable");
var consumeTicket3 = /* @__PURE__ */ __name(async (admin, user, metadata, usageId, ticketCost, corsHeaders2) => {
  const cost = Math.max(1, Math.floor(ticketCost));
  const email = user.email;
  if (!email) {
    return { response: jsonResponse6({ error: "Email not available." }, 400, corsHeaders2) };
  ...
  if (alreadyConsumed && isPipelineUsageId(usageId)) {
    const { data: consumedEvent, error: consumedEventError } = await admin.from("ticket_events").select("user_id, email, created_at").eq("usage_id", usageId).maybeSingle();
    if (consumedEventError) {
      return { response: jsonResponse6({ error: "\u30B5\u30FC\u30D0\u30FC\u5185\u90E8\u30A8\u30E9\u30FC\u304C\u767A\u751F\u3057\u307E\u3057\u305F\u3002" }, 500, corsHeaders2) };
    }
    if (!consumedEvent) {
      return { response: jsonResponse6({ error: "\u30D1\u30A4\u30D7\u30E9\u30A4\u30F3\u8B58\u5225\u5B50\u304C\u7121\u52B9\u3067\u3059\u3002\u518D\u751F\u6210\u3057\u3066\u304F\u3060\u3055\u3044\u3002" }, 409, corsHeaders2) };
    }
    const eventUserId = consumedEvent.user_id ? String(consumedEvent.user_id) : "";
    const eventEmail = normalizeEmail3(consumedEvent.email ? String(consumedEvent.email) : "");
    const userEmail = normalizeEmail3(user.email);
    const ownedByUser = Boolean(eventUserId && eventUserId === user.id || eventEmail && userEmail && eventEmail === userEmail);
    if (!ownedByUser) {
      return { response: jsonResponse6({ error: "Job not found." }, 404, corsHeaders2) };
    }
    ...
    if (!Number.isFinite(createdAtMs) || Math.abs(Date.now() - createdAtMs) > PIPELINE_USAGE_ID_MAX_AGE_MS) {
      return { response: jsonResponse6({ error: "\u30D1\u30A4\u30D7\u30E9\u30A4\u30F3\u8B58\u5225\u5B50\u306E\u671F\u9650\u304C\u5207\u308C\u3066\u3044\u307E\u3059\u3002\u518D\u751F\u6210\u3057\u3066\u304F\u3060\u3055\u3044\u3002" }, 409, corsHeaders2) };
    }
  }
  const ticketsLeft = Number(result?.tickets_left);
  return {
...
var refundTicket3 = /* @__PURE__ */ __name(async (admin, user, metadata, usageId, ticketCost, corsHeaders2) => {
  const refundAmount = Math.max(1, Math.floor(ticketCost));
  const email = user.email;
  if (!email) {
    return { skipped: true };
...
var ensureUsageOwnership = /* @__PURE__ */ __name(async (admin, user, usageId, corsHeaders2) => {
  ...
  const userEmail = normalizeEmail3(user.email);
  const chargeUserId = chargeEvent.user_id ? String(chargeEvent.user_id) : "";
  const chargeEmail = normalizeEmail3(chargeEvent.email ? String(chargeEvent.email) : "");
  const matchesUser = Boolean(chargeUserId && chargeUserId === user.id);
...
var normalizeBase64 = /* @__PURE__ */ __name((value) => {
...
var normalizeVideoExt = /* @__PURE__ */ __name((value) => {
...
var extensionToMime = /* @__PURE__ */ __name((ext) => {
...
var parseOptionalNumber = /* @__PURE__ */ __name((raw) => {
  if (raw === void 0 || raw === null || raw === "") return void 0;
  const n = Number(raw);
  if (!Number.isFinite(n)) return void 0;
  return n;
}, "parseOptionalNumber");
var parseOptionalBoolean = /* @__PURE__ */ __name((raw) => {
  if (raw === void 0 || raw === null || raw === "") return void 0;
...
var buildDefaultWorkflow = /* @__PURE__ */ __name((uploadFilename, prompt, options) => {
  const seed = typeof options?.seed === "number" && Number.isFinite(options.seed) ? Math.max(0, Math.floor(options.seed)) : Math.floor(Math.random() * 2147483647);
  const steps = typeof options?.steps === "number" && Number.isFinite(options.steps) ? Math.max(1, Math.floor(options.steps)) : 25;
  const cfg = typeof options?.cfg === "number" && Number.isFinite(options.cfg) ? Math.max(0, options.cfg) : 4.5;
  ...
  const fixedDuration = useFixedDuration ? Math.max(0.5, options.duration) : null;
  return {
    "85": {
...
var buildMuxWorkflow = /* @__PURE__ */ __name((videoFilename, audioVideoFilename) => {
  return {
    "201": {
      class_type: "VHS_LoadVideo",
      inputs: {
...
var extractRunpodStatus = /* @__PURE__ */ __name((payload) => {
...
var isFailureStatus3 = /* @__PURE__ */ __name((status) => ["FAILED", "CANCELLED", "TIMED_OUT", "ERROR"].includes(String(status || "").toUpperCase()), "isFailureStatus");
var extractRunpodJobId = /* @__PURE__ */ __name((payload) => {
  const raw = payload?.id ?? payload?.job_id ?? payload?.jobId ?? payload?.output?.id ?? payload?.output?.job_id;
  return raw ? String(raw) : "";
}, "extractRunpodJobId");
var extractError = /* @__PURE__ */ __name((payload) => payload?.error || payload?.message || payload?.detail || payload?.output?.error || payload?.output?.message || payload?.result?.error || payload?.result?.message, "extractError");
var extractVideoOutput = /* @__PURE__ */ __name((payload, options) => {
  const roots = [
    payload,
    payload?.output,
    payload?.result,
  ...
  const preferred = (normalizedPrefix ? candidates.find((candidate) => candidate.outputFilename?.toLowerCase().startsWith(normalizedPrefix)) : null) ?? candidates[0];
  return {
...
var requestRunpod = /* @__PURE__ */ __name(async (endpoint, path, apiKey, init = {}) => {
...
var onRequestPost6 = /* @__PURE__ */ __name(async ({ request, env }) => {
  ...
  const auth = await requireGoogleUser5(request, env, corsHeaders2);
  if ("response" in auth) return auth.response;
  const runpodApiKey = env.RUNPOD_API_KEY?.trim();
  const endpoint = resolveEndpoint3();
  if (!runpodApiKey || !endpoint) {
    return jsonResponse6({ error: ERR_SERVER_CONFIG }, 500, corsHeaders2);
  }
  const payload = await parseJsonSafe(request);
  if (!payload || typeof payload !== "object") {
    return jsonResponse6({ error: ERR_INVALID_BODY }, 400, corsHeaders2);
  }
  const input = payload.input ?? payload;
  ...
  if (typeof rawPipelineUsageId === "string" && rawPipelineUsageId.trim() && !pipelineUsageId) {
    return jsonResponse6({ error: "pipeline_usage_id is invalid or expired." }, 400, corsHeaders2);
  }
  if (input?.workflow) {
    return jsonResponse6({ error: "workflow overrides are not allowed." }, 400, corsHeaders2);
  }
  const isMuxOnly = parseOptionalBoolean(input?.mux_only ?? input?.muxOnly) === true;
  if (isMuxOnly) {
    if (!pipelineUsageId) {
      return jsonResponse6({ error: "pipeline_usage_id is required for mux_only." }, 400, corsHeaders2);
    }
    const ownership = await ensureUsageOwnership(auth.admin, auth.user, pipelineUsageId, corsHeaders2);
    ...
    if (!baseVideoBase64) {
      return jsonResponse6({ error: ERR_MUX_VIDEO_REQUIRED }, 400, corsHeaders2);
    }
    const audioVideoBase64 = normalizeBase64(
      input?.audio_video_base64 ?? input?.audioVideoBase64 ?? input?.audio_video ?? input?.audioVideo
    );
    if (!audioVideoBase64) {
      return jsonResponse6({ error: ERR_MUX_AUDIO_VIDEO_REQUIRED }, 400, corsHeaders2);
    }
    const baseVideoBytes = estimateBase64Bytes2(baseVideoBase64);
    const audioVideoBytes = estimateBase64Bytes2(audioVideoBase64);
    if (baseVideoBytes > MAX_VIDEO_BYTES2 || audioVideoBytes > MAX_VIDEO_BYTES2 || baseVideoBytes + audioVideoBytes > MAX_VIDEO_BYTES2 * 2) {
      return jsonResponse6({ error: ERR_VIDEO_TOO_LARGE }, 413, corsHeaders2);
    }
    ...
    if (isFailureStatus3(muxStatus) || extractError(muxRunResult.payload)) {
      return jsonResponse6(
        {
    ...
    if (!muxVideo) {
      return jsonResponse6({ error: ERR_MUX_FAILED, status: muxStatus }, 502, corsHeaders2);
    }
    return jsonResponse6(
      {
        status: muxStatus,
  ...
  const ticketCheck = await ensureTicketAvailable2(auth.admin, auth.user, MMAUDIO_TICKET_COST, corsHeaders2);
  if ("response" in ticketCheck) return ticketCheck.response;
  let runpodInput = {};
  {
    const text = String(input?.text ?? input?.prompt ?? "").trim();
    if (!text) {
      return jsonResponse6({ error: ERR_PROMPT_REQUIRED }, 400, corsHeaders2);
    }
    if (text.length > MAX_PROMPT_LENGTH3) {
      return jsonResponse6({ error: ERR_PROMPT_TOO_LONG }, 400, corsHeaders2);
    }
    const videoBase64 = normalizeBase64(input?.video_base64 ?? input?.videoBase64 ?? input?.video);
    if (!videoBase64) {
      return jsonResponse6({ error: ERR_VIDEO_REQUIRED }, 400, corsHeaders2);
    }
    if (estimateBase64Bytes2(videoBase64) > MAX_VIDEO_BYTES2) {
      return jsonResponse6({ error: ERR_VIDEO_TOO_LARGE }, 413, corsHeaders2);
    }
  ...
  try {
    ...
    return jsonResponse6({ error: ERR_REQUEST_FAILED }, 502, corsHeaders2);
  }
  if (!runResult.ok) {
    return jsonResponse6(
      {
  ...
  const usageId = pipelineUsageId || (id ? `mmaudio:${id}` : `mmaudio:adhoc:${makeUsageId3()}`);
  const chargeMeta = {
    source: "run",
    job_id: id || null,
    status,
  ...
  const charge = await consumeTicket3(auth.admin, auth.user, chargeMeta, usageId, MMAUDIO_TICKET_COST, corsHeaders2);
  if ("response" in charge) return charge.response;
  let ticketsLeft = Number.isFinite(Number(charge.ticketsLeft)) ? Number(charge.ticketsLeft) : null;
  if (isFailureStatus3(status) || extractError(runResult.payload)) {
    const refundMeta = {
      source: "run",
      job_id: id || null,
      status,
    ...
    const refund = await refundTicket3(auth.admin, auth.user, refundMeta, usageId, MMAUDIO_TICKET_COST, corsHeaders2);
    if ("response" in refund) return refund.response;
    const nextTickets = Number(refund.ticketsLeft);
...
var onRequestGet4 = /* @__PURE__ */ __name(async ({ request, env }) => {
  ...
  if (!id) {
    return jsonResponse6({ error: "id is required." }, 400, corsHeaders2);
  }
  ...
  try {
    ...
    return jsonResponse6({ error: ERR_STATUS_FAILED }, 502, corsHeaders2);
  }
  if (!statusResult.ok) {
    return jsonResponse6(
      {
  ...
  let ticketsLeft = null;
  if (isFailureStatus3(status) || extractError(payload)) {
    const refundMeta = {
      source: "status",
      job_id: id,
      status,
  ...
  return jsonResponse6(
    {
      ...
      error: isFailureStatus3(status) ? ERR_REQUEST_FAILED : null
    },
    200,
    corsHeaders2
  );
...
// api/tickets.ts
var SIGNUP_TICKET_GRANT4 = 3;
var corsMethods6 = "GET, OPTIONS";
var INTERNAL_SERVER_ERROR_MESSAGE4 = "\u30B5\u30FC\u30D0\u30FC\u5185\u90E8\u30A8\u30E9\u30FC\u304C\u767A\u751F\u3057\u307E\u3057\u305F\u3002\u6642\u9593\u3092\u304A\u3044\u3066\u518D\u5EA6\u304A\u8A66\u3057\u304F\u3060\u3055\u3044\u3002";
var ERROR_LOGIN_REQUIRED4 = "\u30ED\u30B0\u30A4\u30F3\u304C\u5FC5\u8981\u3067\u3059\u3002";
var ERROR_AUTH_FAILED4 = "\u8A8D\u8A3C\u306B\u5931\u6557\u3057\u307E\u3057\u305F\u3002";
var ERROR_GOOGLE_ONLY4 = "Google\u30ED\u30B0\u30A4\u30F3\u306E\u307F\u5BFE\u5FDC\u3057\u3066\u3044\u307E\u3059\u3002";
var ERROR_SUPABASE_NOT_SET4 = "SUPABASE_URL \u307E\u305F\u306F SUPABASE_SERVICE_ROLE_KEY \u304C\u8A2D\u5B9A\u3055\u308C\u3066\u3044\u307E\u305B\u3093\u3002";
var ERROR_EMAIL_MISSING2 = "\u30E1\u30FC\u30EB\u30A2\u30C9\u30EC\u30B9\u304C\u53D6\u5F97\u3067\u304D\u307E\u305B\u3093\u3002";
...
var fetchTicketRow4 = /* @__PURE__ */ __name(async (admin, user) => {
  ...
  const { data: byEmail, error: emailError } = await admin.from("user_tickets").select("tickets").ilike("email", email).maybeSingle();
  if (emailError) {
    return { error: emailError };
  }
  return { data: byEmail, error: null };
...
var isGoogleUser6 = /* @__PURE__ */ __name((user) => {
  if (user.app_metadata?.provider === "google") return true;
  if (Array.isArray(user.identities)) {
    return user.identities.some((identity) => identity.provider === "google");
  }
...
var requireGoogleUser6 = /* @__PURE__ */ __name(async (request, env, corsHeaders2) => {
  const token = extractBearerToken6(request);
  if (!token) {
    return { response: jsonResponse7({ error: ERROR_LOGIN_REQUIRED4 }, 401, corsHeaders2) };
  }
  const admin = getSupabaseAdmin7(env);
  if (!admin) {
    return { response: jsonResponse7({ error: ERROR_SUPABASE_NOT_SET4 }, 500, corsHeaders2) };
  }
  const { data, error } = await getSupabaseUserWithRetry(admin, token);
  if (error || !data?.user) {
    return { response: jsonResponse7({ error: ERROR_AUTH_FAILED4 }, 401, corsHeaders2) };
  }
  if (!isGoogleUser6(data.user)) {
    return { response: jsonResponse7({ error: ERROR_GOOGLE_ONLY4 }, 403, corsHeaders2) };
  }
  return { admin, user: data.user };
}, "requireGoogleUser");
...
var onRequestGet5 = /* @__PURE__ */ __name(async ({ request, env }) => {
  ...
  const auth = await requireGoogleUser6(request, env, corsHeaders2);
  if ("response" in auth) {
    return auth.response;
  }
  const email = auth.user.email;
  if (!email) {
    return jsonResponse7({ error: ERROR_EMAIL_MISSING2 }, 400, corsHeaders2);
  }
  const { data, error } = await ensureTicketRow4(auth.admin, auth.user);
  if (error) {
    return jsonResponse7({ error: INTERNAL_SERVER_ERROR_MESSAGE4 }, 500, corsHeaders2);
  }
  return jsonResponse7({ tickets: data?.tickets ?? 0, hasRecord: Boolean(data) }, 200, corsHeaders2);
}, "onRequestGet");

// ../.wrangler/tmp/pages-Gf5mVo/functionsRoutes-0.15786638814961484.mjs
var routes = [
  {
  ...
  {
    routePath: "/_shared/wan-lora-pack-handler",
    mountPath: "/_shared",
    method: "GET",
    middlewares: [],
